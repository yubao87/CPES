				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<h3 class="panel-title">
							<i class="glyphicon glyphicon-th"></i> 数据列表 数据列表
					...
					<div class="panel-body">
						...
						<button type="button" onclick="deleteRoles()" id="batDeleteBtn" class="btn btn-danger"
							style="float: right; margin-left: 10px;">
							<i class=" glyphicon glyphicon-remove"></i> 删除
						</button>
						<button type="button" class="btn btn-primary" style="float: right;" onclick="window.location.href='${APP_PATH }/role/add.htm'">
	...
	<script type="text/javascript">
		$(function() {
			...
			$("#batDeleteBtn").click(function(){
				...
				if(roles.length > 0 ){
					layer.confirm("删除选择的角色信息是否继续?",  {icon: 3, title:'提示'}, function(cindex){
...
// 						});
						...
						$.each(roles,function(index,role){
// 							dataObj["roles[" + index + "].id"] = role.value;
							dataObj["ids[" + index + "]"] = role.value;
						});
						//删除角色信息
					    $.ajax({
					    	url : "${APP_PATH}/role/doDeletes.do",
					    	type : "POST",
					    	data : dataObj,
					    	success : function(result){
					    		if(result.success){
					    			layer.msg("选择的角色信息删除成功", {time:1500, icon:6},function(){
					    				pageQuery(0);
					    			});
					    		} else {
					    			layer.msg("选择的角色信息删除失败", {time:1500, icon:5});
					    		}
					    	}
					    });
						layer.close(cindex);
...
// 				paramObj.queryText = queryText.val();
			...
			$.ajax({
				...
				success : function(result){
					...
					if(result.success){
						...
						//循环遍历角色数据
						//index当前循环的索引，role 当前循环的临时变量
						var concat = "";
						$.each(pageObj.datas,function(index, role){
							concat += '<tr>';
							concat += '	<td>' + (index+1) + '</td>';
							concat += '	<td><input type="checkbox" value="' + role.id + '"></td>';
							concat += '	<td>' + role.name + '</td>';
							concat += '	<td>';
							concat += '		<button type="button" onclick="window.location.href=\'${APP_PATH}/role/assign/' + role.id + '.htm\'" class="btn btn-success btn-xs">';
							concat += '			<i class=" glyphicon glyphicon-user"></i>';
							concat += '		</button>';
							concat += '		<button type="button" onclick="window.location.href=\'${APP_PATH}/role/edit/' + role.id + '.htm\'" class="btn btn-primary btn-xs">';
							concat += '			<i class=" glyphicon glyphicon-pencil"></i>';
							concat += '		</button>';
							concat += '		<button type="button" onclick="deleteRole(' + role.id + ', \'' + role.name + '\')" class="btn btn-danger btn-xs">';
		...
		function deleteRole(id,rolename){
			layer.confirm("删除【" + rolename + "】的角色信息是否继续?",  {icon: 3, title:'提示'}, function(cindex){
			    //删除角色信息
			    $.ajax({ 
			    	...
			    	success : function(result){
			    		if(result.success){
			    			layer.msg("角色【" + rolename +"】删除成功", {time:1500, icon:6},function(){
			    				pageQuery(0);
			    			});
			    		}
			    	}
